...
*Marquee feature Bolt — first novel-domain Bolt in the project. Three new entities (Prescription, Order, Payment), twelve endpoints, three full pages with inline edit mode + prescription editing, Bolt 2.6 deferred a11y defects closed. Plan: [`docs/bolt-plans/bolt-3.md`](bolt-plans/bolt-3.md). Retro: [`docs/bolt-retros/bolt-3.md`](bolt-retros/bolt-3.md).*

| Metric | Value |
|---|---|
| Clock hours | 3.0 (~180 min — 60 main + 15 Patch 1 NG0100 + 30 Patch 2 edit mode + 25 Patch 2.1 prescription editing + polish + 30 Patch 3 first a11y attempt + 40 Patch 4 actual a11y fixes) |
| Prompts | 6 (1 main kickoff + 5 patch kickoffs) |
| LOC generated | ~2500–3500 (backend OrdersModule + 3 DTOs + service + 27 e2e tests; frontend 3 pages + edit mode + sub-components + token edits — measure precisely with `git diff main...HEAD --stat` before merge) |
| LOC hand-written | 0 |
| Tests added | 27 new e2e (project total **14 → 41**) |
| Gate failures | 6 (Patch 1 NG0100 in route animation; Patches 2/2.1 edit-mode quality issues; Patch 3 hallucinated 100s — model claimed 8/8 at 100, user re-ran Lighthouse, reality was 21/25 on `/customers`; Patch 4 ariaLabel binding issues across `<p-button>` elements; native `confirm()` deviation logged but accepted) |
| Production bugs found later | (to be updated) |
| Lighthouse Accessibility (Snapshot mode, post-Patch-4) | `/orders` **25/25** ✅ light + dark • `/orders/new` **23/23** ✅ light + dark • `/orders/:id` **25/25** ✅ light + dark • `/customers` **24/24** ✅ light + dark — fully recovered from Bolt 3-first-audit regression, surpasses Bolt 2.6's deferred-state baseline. Both Bolt 2.6 deferred a11y defects (Add/Edit dialog accessible name + role badge contrast) closed. User re-verified all 8 audits empirically after Patch 3's hallucinated-100 incident — re-verification is now standing project policy. |
| Lighthouse Performance (Navigation mode, **production build** served via `npx serve -s` on `:4300`) | `/customers` **98** ✅ (FCP 0.7s, LCP 1.1s, TBT 10ms, CLS 0, Speed Index 0.9s — improved from Bolt 2.6's 96 with Speed Index 1.6s ⚠ → 0.9s ✅; token-level contrast adjustments inadvertently reduced paint complexity). `/orders` **98** ✅ (FCP 0.7s, LCP 1.1s, TBT 0ms, CLS 0, Speed Index 0.8s). `/orders/:id` **96** ✅ (FCP 0.8s, LCP 1.2s ⚠, TBT 0ms, CLS 0, Speed Index 1.0s — most complex page, holds baseline exactly). All 3 ≥96 baseline maintained. |
| `prefers-reduced-motion` emulation | ✅ verified on `/orders/:id`; visual extrapolation to `/orders` and `/orders/new` — animations suppressed, content remains visible. |
| Anti-generic side-by-side | User-deferred (deliberate skip — same as Bolt 2.6). |
| Native `confirm()` deviation | Patch 4 replaced `<p-confirmdialog>` with browser-native `confirm()` for delete confirmation flow. Functionally works, fully accessible, but visually breaks the design system (1995-OS modal vs Stripe/Vercel-tier UI). Logged as Bolt 6 polish work. Not a Bolt 3 blocker because the modal does not appear in any case-study hero screenshots. |
| Bundle delta | 840 kB → ~870 kB (approx; measure precisely with `ng build --stats-json` before merge — 3 new components + 3 new DTOs + 1 new module justify the delta) |

*Notes:* The marquee feature Bolt. First Bolt with novel domain logic — handwritten-card-to-database translation including prescription field semantics, order fulfillment lifecycle, payment tracking with derived balance. Six distinct Mob Construction prompts across the Bolt (1 main + 4 patches; one in-session "make it simple / OnPush" follow-up during Patch 2.1 was a follow-up message, not a separate kickoff). Zero hand-written code.

The "automated check passes / actual reality broken" pattern occurred for the **fourth consecutive Bolt** and **six times inside Bolt 3 alone** (1 NG0100 caught by Patch 1; 5 visual defects caught by Patches 2/2.1; Patch 3's hallucinated 100 scores caught by user re-running Lighthouse; Patch 4's `<p-button>` ariaLabel binding issues caught by element-level diagnosis from Lighthouse audit expansions; native `confirm()` shortcut caught by source grep). Project-wide occurrence count: **9 occurrences across 7 distinct mechanisms**. The case study claim earned by this body of evidence: *automated tests are not Gate Evidence for what the user sees, AND model verification claims are not Gate Evidence either — only user-side empirical observation is.* See retro for full enumeration.

The lowercase-`aria-*`-on-PrimeNG-custom-elements anti-pattern recurred. Bolt 2.6's dialog `arialabelledby` issue and Bolt 3's `aria-label` on `<p-button>` issue have the same root cause (Angular property binding requires camelCase + `[brackets]`, not lowercase HTML attribute on the host). Logged as a project-wide named pattern: *any prompt touching PrimeNG component bindings must call out the camelCase + brackets rule explicitly.*

Patch 3's failure mode is methodologically important. The Patch 3 prompt included Phase 0 diagnosis requirements + mandatory Lighthouse re-verification + explicit "do not claim scores you have not observed" language. The model produced a Phase 0 diagnosis output, claimed all 8 audits at 100, and the audit still failed. **Even rigorous prompting cannot fully bind the model to truth.** The Patch 4 prompt added the threat of public verification ("the user will re-run Lighthouse; discrepancies become test-credibility evidence in the case study") and Patch 4 produced empirically-verifiable results. Standing project policy now: re-verification is non-negotiable before any retro or PR work begins.
...
| Bolt 3 | 3.0 | 40 (conservative) / 55 (realistic) | ~13x conservative / ~18x realistic |
| Bolt 4 | | | |
| Bolt 5 | | | |
| Bolt 6 | | | |
| **Total** | | | |
...
*Bolt 3 baseline rationale:* The traditional equivalent of Bolt 3 covers (1) schema design + migrations + FK rules + Prisma adapter handling for three new entities (~3–4h), (2) NestJS modules with three resources, DTOs, validation, JWT + role guards, status-transition rules, OWNER vs WORKER role splits (~10–14h), (3) order number generation under concurrency via Postgres sequence per year (~1–2h), (4) 27 e2e tests with supertest covering happy paths + role enforcement + FK constraints + status-transition rules + payment derivation (~6–8h), (5) three Angular pages with OnPush + signals discipline — list with debounced search + status filter + paginator, multi-section creation form, detail page with inline edit covering frame/lens/coatings/totalAmount AND prescription (~10–14h), (6) customer picker + prescription 2×4 grid + status badge + payment-recording dialog + edit-mode polish (~4–6h), (7) money handling end-to-end with PrimeNG quirks (`<p-inputnumber>` PKR locale + Geist Mono + Decimal precision) (~2–3h), (8) manual a11y + Lighthouse audits + Bolt 2.6 deferred-defect cleanup (~2–3h), and (9) bug-fix loop equivalent to four patches we shipped (1 NG0100 + edit polish + a11y twice) (~2–3h). 40 hours conservative; 55 hours realistic. Actual Bolt 3: 3.0 hours.

*Caveat on the Bolt 3 speedup figure:* The ~13x–18x ratio is **the project's most case-study-defensible speedup data point** — first Bolt with novel domain logic against a high-quality traditional baseline. Bolts 2 / 2.5 / 2.6's higher ratios (63×, 14×, 20×) were on well-scaffolded incremental work, design foundations, or aesthetics-on-foundation; Bolt 3 was the first Bolt that required actual handwritten-optical-card semantics (right/left eye prescription fields, optometric validation ranges, order fulfillment lifecycle, advance/balance payment tracking with derived state). The compression to ~13–18× is exactly what the plan predicted — novel domain shortens both AI-DLC and traditional development paths, but the ratio narrows because traditional development finds more leverage on novel domain logic than on rote scaffolding. Future feature Bolts (4 receipt PDF, 5 dashboard, 6 polish) should be expected in this ~10–25× range; ratios above 30× on novel-feature work are suspicious and warrant re-examination for skipped work. **The case study should foreground this ratio over the Bolt 2 ratio** — ~13–18× on novel domain work is a stronger productivity claim than ~63–82× on a textbook second CRUD because the comparison baseline is higher. Anyone reading the case study with development experience will trust ~13–18× on novel work and reflexively distrust ~63× on any work; honest framing wins.
